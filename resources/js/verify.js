 ...
 */
Verifys.checkEmail = function(strEmail) {
 ...
 */
Verifys.checkMobile = function (strMobile) {
	var regu = /^[1][3][0-9]{9}$/;
 ...
 */
Verifys.checkPhone = function (strPhone) {
 ...
 */
Verifys.isInteger = function (str) {
	var regu = /^[-]{0,1}[0-9]{1,}$/;
 ...
 */
Verifys.isDecimal = function (str) {
	if (isInteger(str)) {
		return true;
	}
	var re = /^[-]{0,1}(\d+)[\.]+(\d+)$/;
 ...
 */
Verifys.isNumberOr_Letter = function (s) {
	// 判断是否是数字或字母
 ...
 */
Verifys.isNumberOrLetter = function (s) {
	// 判断是否是数字或字母
	var regu = "^[0-9a-zA-Z]+$";
	var re = new RegExp(regu);
	if (re.test(s)) {
 ...
 */
Verifys.isChinaOrNumbOrLett = function (s) {
	// 判断是否是汉字、字母、数字组成
 ...
 */
Verifys.isDate = function (date, fmt) {
	if (fmt == null) {
		fmt = "yyyyMMdd";
	}
...
Verifys.getMaxDay = function (year, month) {
 ...
 */
Verifys.isLastMatch = function (str1, str2) {
 ...
 */
Verifys.isFirstMatch = function (str1, str2) {
 ...
 */
Verifys.isMatch = function (str1, str2) {
	var index = str1.indexOf(str2);
	if (index == -1) {
		return false;
	}
 ...
 */
Verifys.checkTwoDate = function (startDate, endDate) {
	if (!isDate(startDate)) {
		alert("起始日期不正确!");
		return false;
 ...
 */
Verifys.checkSelect = function (checkboxID) {
	var check = 0;
	var i = 0;
...
Verifys.getTotalBytes = function (varField) {
	if (varField == null) {
		return -1;
	}
	var totalCount = 0;
...
Verifys.getFirstSelectedValue = function (checkboxID) {
	var value = null;
	var i = 0;
	if (document.all(checkboxID).length > 0) {
		for (i = 0; i < document.all(checkboxID).length; i++) {
...
Verifys.getFirstSelectedIndex = function (checkboxID) {
	var value = -2;
	var i = 0;
	if (document.all(checkboxID).length > 0) {
		for (i = 0; i < document.all(checkboxID).length; i++) {
...
Verifys.selectAll = function (checkboxID, status) {
...
Verifys.selectInverse = function (checkboxID) {
	if (document.all(checkboxID) == null) {
		return;
	}
	if (document.all(checkboxID).length > 0) {
...
Verifys.checkDate = function (value) {
	if (value == '') {
		return true;
	}
...
/**
 * 用途：检查输入的起止日期是否正确，规则为两个日期的格式正确或都为空且结束日期>=起始日期 输入：startDate：起始日期，字符串; endDate：
 * 结束日期，字符串 返回：如果通过验证返回true,否则返回false
 */
Verifys.checkPeriod = function (startDate, endDate) {
 ...
 */
Verifys.checkSecCode = function (secCode) {
	if (secCode.length != 6) {
		alert("证券代码长度应该为6位");
		return false;
	}
 ...
 * value:去掉空格的字符串
 */
Verifys.cTrim = function (sInputString, iType) {
	var sTmpStr = ' ';
	var i = -1;
	if (iType == 0 || iType == 1) {
		while (sTmpStr == ' ') {
 ...
 * @return {}
 */
Verifys.checkKeyForInt = function(value, e) {
	var isOK = false;
 ...
 * @return {}
 */
Verifys.checkKeyForFloat = function(value, e) {
	var isOK = false;
	var key = window.event ? e.keyCode : e.which;
	if ((key > 95 && key < 106) || // 小键盘上的0到9
			(key > 47 && key < 60) || // 大键盘上的0到9
